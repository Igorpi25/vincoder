...
Как бы я организовал mvp слой в простой случае, когда не нужен Pager? Посмотрим модули
1. Есть модуль ui-kit
4. Есть модуль app
5. Есть модуль network. Это реализация контрактов repository
6. Есть модуль db. Реализация котрактов repository
7. Есть repository модуль где торчат интерфейсы контрактов
8. feature модули. 

Модули должны быть взаимозаменяемы, в этом суть. 

Одинаковая сигнатура PagingSource и RemoteMediator дает нам ...

### Почему мне не нравится модули -api и -impl?

Такое ощущение, что мы грубо разделили "кошку пополам"(из комментариев к "Северным рассказам" Владимира Серкина). Это очень похоже на группировку классов по формальному признаку: все репозитоии в одной папке, все модели в одной папке, и т.д. Это как взять десять машин, и в одну кучу положить их моторы, в одну кучу раму, в другую колеса. Но так оно не работает. Так оно не ездит.

Как бы ты их разделил тогда? Ну, во первых в этом должна быть не структурный замысел, а функциональная необходимость. Модуль однозначно должен быть цельным, только его скелет отдельно и мясо к нему, никакой практической пользы не принесут. Это будет скелет мутанта притянутый "за уши", к нему подойдет только свое мясо.

Мы должны отдельный в интерфейс некую "потребность", в котором нуждается модуль потребитель. Т.е. контракт выпускает именно потребитель, а не "поставщик". В методе разделения Касперского, мы наблюдаем пример когда feature выдает контракт под себя, для того чтобы самому-же его исполнить.


Как в эту концепцию ложится Paging? 

### Манифест многомульности
Многомодульность не так просто нужна, и я хочу обозначить моменты, для чего именно:
1. Возможность изолированного запуска отдельных фич, в виде приложения на телефоне. Это непростая задача, но ее нужно все время решать, и непрерывно поддерживать. В этом случае, все остальные задачи могут быть решении как следствие решения этой задачи. Например, полноценная тестируемость. Это придаст возможность независимой работы над разными модулями. Например, мы можем позволить себе команду, которая работает только над модулем ui-kit.
2. Обратная поддержка версий модулей. Обратная поддержка версий контрактов модулей. Особенно актуально при кросс модульных фичах и модулях. 
3. Изоляция. Все что торчит наружу(не internal), подлежит к поддержке версий и обратной совместимости.
4. Поддержка warning и alert при кросс-модульных конфликах, и конфликах вложенностей.
6. Реальная взаимозаменяемость сторонних библиотек: API, DB
7. Возможность перехода на Compose? ну-ну, посмотрим
8. По возможности возможность замены DI
9. Скорость сборки. Насколько это будет работать?
10. Удобная локализация. Фиг знает. 
11. UI-kit примеры. Возможность верстки и запуска отдельной "ui-фичи" до начала разработки. И ui тесты к нему. Во время разработки нужно будет работать 
12. UseCase в качестве отдельной "фичи", с unit-test. Получится так выделить бизнес логику?  посмотрим

Таким образом, мы несем определенные ценности, которых нужно придерживаться и поддерживать.
